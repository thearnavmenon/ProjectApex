...
| 2026-06-11 | Train locked (`docs/design/train.md`) after the four-agent panel — the fifth and final per-screen spec. **The program root is a measured vertical day-spine** (time axis = the left datum, week position over it; Progress's floor-spine analog), and **the generation horizon is a *drawn* datum** — a drafting-rule break with a "to-be-placed" dashed/hatch skeleton zone and a distance commitment-gradient — so Option-4's diminishing knowledge (skeleton far out, numbers near) is drawn, not a colour swap. **Day status reuses the filled/hollow/undrawn tick vocabulary** (done = a logged fact, never a check/ring/counter — the calendar is a plan, not a report card). **The exercise leaf is one scroll, draws no band** (points up via a list-scale band micro-mark — one-component-three-contexts held at three) and **no per-exercise chart** (killing the "if shown" seam that readmitted Hevy's banned slope to the exercise surface). **Plan agency = level (b)** — reschedule / "can't train today" + swap-variant-within-pattern, reusing the loop's swap surface, rendered in the `AMENDED` provenance vocabulary. **Train hosts the calibration-review surface** (re-calibration changes the program; retires the ownerless #269/#305 build debt). Regeneration-under-the-eye hard-swaps assembled (no tween/cascade/flourish); the reasoning line renders assembled (Train inherits no bookend); "one *why* register, three scales". Muscle heatmaps/body-maps cut by name. **No new tokens** — Train reuses existing instruments wholesale. |
