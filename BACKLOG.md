...
- [x] P5-D02: #221 — adopt `USER-REPORTED SIGNALS` section. **DONE 2026-06-08** (PR #278). Full section adopted (pain one-strike / form_breakdown / intent-deviation). Input already shipped in the payload (within_session_performance / current_exercise_sets_today) — text-only. Fixed the stale `session_log` wording; within-session only (cross-session = #43).
- [x] P5-D03: #222 — equipment-aware weight increments. **DONE 2026-06-08** (PR #280). Adopted just the load-bearing delta: weight-stack/plate-loaded machines step 5 kg (not 2.5), round to nearest 5 — matches `DefaultWeightIncrements.machineStack`. Prompt-only; deterministic code-side snap deferred to **#279**. Cross-cutting flag: `cableStack` is also 5 kg in code but the prompt keeps cable at 2.5 kg (debatable — micro-plates) — left for a separate decision.
...
- [ ] P5-D05: Phase 2 follow-ups — `#167` and `#151` closed in the functional-defect sweep (§2E); **`#166` (per-axis confidence lifecycle — all 3 axes) DONE 2026-06-08** (PRs #287–#291, ADR-0020; umbrella #166 closed — see §2G). `#164`, `#165` (MuscleProfile `volumeTolerance` cadence-scaling / EWMA-update) remain open — deliberately independent of #166 (muscle confidence aggregates from its patterns, not its own volume signal).
...
- [x] P5-D10: #269 — calibration-review projections (derive + review + edit). **SHIPPED & CLOSED 2026-06-09** (5 slices #294–#298, PRs #299–#302 + docs; ADR-0021; umbrella #269 closed). See §2H. Spinoff for later: goal-renegotiation stretch re-derivation (deferred).
- [ ] P5-D11: #292 — `PatternProfile.recentSessionDates` grows unbounded in the EF write (no `.slice(-N)` cap; found during #284). Recommend bounding to last ~10 on write after confirming no consumer needs the full history.
...
### 2G — Per-axis confidence lifecycle (#166, all 3 axes) (2026-06-08)

The keystone Phase-2 follow-up. `AxisConfidence` (`bootstrapping → calibrating → established → seasoned`) gates how much the coach trusts each axis, but **no axis ever advanced** — every exercise/pattern/muscle bootstrapped to `.bootstrapping` and stayed there forever (ADR-0005 designed a calibration review but the transitions were never built). A code audit corrected two false premises in the inbound handoff: (1) there was *no* existing exercise/pattern lifecycle to copy as a template, and (2) #269's real blocker is *pattern* confidence, not muscle. With that, scope was set to **all three axes**.

Design via the mandated three-input best-of `grill-me` (per-question: my recommendation + a fresh clean-slate reviewer + a cumulative memory-carrying reviewer; 4 dependency-ordered levels, 10 questions). Decisions pinned in **ADR-0020**. Built as 5 tracer-bullet slices, branch-per-slice, auto-merged on `ef-test` green.

- [x] Slice 1: #287 (#282) — foundation: ADR-0020 + `monotonicAdvance` clamp (forward-only, no-skip) + 3-case write-contract type (makes a `seasoned` write a compile error) + property test.
- [x] Slice 2: #288 (#283) — exercise axis: calibrating @ `sessionCount≥3` & ≥3 valid top sets; established @ `sessionCount≥8` & e1RM CV≤7.5% over last 5 distinct sessions (≥4-session guard). Generalized `transitionModeMean` with a backward-compatible `window` param.
- [x] Slice 3: #289 (#284) — EF-only per-pattern `sessionCount` + idempotent backfill (seed from `recentSessionDates.length`). Grep-and-report → #292 (recentSessionDates unbounded in the EF write).
- [x] Slice 4: #290 (#285) — pattern axis: established @ `sessionCount≥6` & a data-backed trend (`isTrendEvaluable`, since `plateauVerdict` returns "progressing" as both the no-data default and a real verdict). **Flips `isReadyForCalibrationReview` at ≥4/6 major → unblocks #269.** Advance-only boundary asserted (never writes projections).
- [x] Slice 5: #291 (#286) — muscle axis: aggregates from participating patterns (≥`ceil(2/3)` established over the FULL participating set incl. isolation, so biceps — zero major patterns — can establish; empty-set guarded).

Key calls: muscle aggregates from patterns (so #164/#165 stay independent); strict forward-only ratchet (a downgrade would retract #269's projections); `seasoned` reserved-unused; **no SQL migration, no Swift change** (the one new field is EF-only JSONB; Swift ignores unknown keys; confidence fields already decode). Verification: 81 `_shared` unit tests + orchestrator integration tests (CI `ef-test` green on every slice). EF live via deploy on merge. Closed umbrella #166. Spinoffs: #292 (P5-D11, unbounded recentSessionDates); #164/#165 remain open; #269 now unblocked.

### 2H — Calibration-review projections (#269) (2026-06-09)

The payoff #166 unblocked. ADR-0005 designed per-pattern `floor`/`stretch` capability targets "set at calibration review" but the layer was never built — `patternProjections` stayed `[]` and `isReadyForCalibrationReview` was inert. With pattern confidence now reaching `.established` (#166), #269 builds it end-to-end: derive → review → edit. Scope set to the full feature (user-approved); design via the three-input best-of grilling (4 questions/2 levels; ADR-0021). A grilling-time discovery: `ExerciseProfile.e1rmMedian`/`e1rmPeak` are dead fields (always 0), so every formula was re-grounded on the live `patternE1RMSessions` series — a peak-anchored stretch idea would have computed against zero.

- [x] S1: #299 (#294) — EF derivation at the pipeline tail: once ≥4/6 majors `.established`, per-pattern lazy-derive floor/stretch/progress for established majors (floors immovable; late-maturing majors picked up later), set `calibrationReviewFiredAt` once, recompute progress each apply. floor = round-down median of recent window; stretch = floor×(1+margin(trend)); 6 majors only. Live inputs only. 12 unit + orchestrator integration.
- [x] S2: #300 (#295) — iOS read-only `CalibrationReviewView` + one-time banner (`deriveCalibrationReviewSignal`, precedence over heavy-reassessment) + local ack. xcodebuild build + tests green.
- [x] S3: #301 (#296) — EF `update-trainee-goal` `stretch_edits`: `FOR UPDATE` upward-only clamp (`max(stored, edited)`), floor untouched, per-leaf jsonb_set (race-safe vs session-apply). 7 validation + 5 DB integration.
- [x] S4: #302 (#297) — iOS upward-only stretch editing + durable ack: EF `acknowledge_calibration_review` persists `calibrationReviewAcknowledged` server-side (banner survives a session sync); Save POSTs edits + ack, mirrors locally. 27 EF + 6 iOS payload tests.
- [x] S5: #298 — ADR-0021 + this entry + diary.

Key calls: floor = recent demonstrated capability (immovable, never overstated); stretch upward-editable, trend-scaled; 6 major patterns only (lunge/isolation deferred); advance/derive-only boundary respected; **no SQL migration**. Projections flow to the LLM automatically (already an encoded digest field). iOS views are compile- + logic-verified, NOT visually QA'd (no simulator render this session) — worth an on-device once-over. Deferred spinoff: goal-renegotiation stretch re-derivation. Closed umbrella #269.

### 2I — Goal-renegotiation stretch re-derivation (#304) (2026-06-09)

The lifecycle event #269/ADR-0021 deferred. ADR-0005 said a pattern's **stretch** is "re-derived silently on goal renegotiation" (floor immovable) and `goalLastRenegotiatedAt` is stamped — but it was never built; the field existed and was never written. Design grilled, then cross-checked by an independent architecture agent. The grilling's central finding reshaped scope: the ADR-0021 stretch formula reads **only floor + trend**, never the goal text, so re-derivation is **nearly inert** — it moves a number only when trend shifted. ADR-0005's word "**silently**" is the tell that this was the intent (model hygiene, not an athlete-visible event). Shipped that honest lifecycle-closure; the goal-**aware** version (where the new goal moves targets) and the floor-step-up question were spun off to **#305**.

- [x] #304 (PR #306) — `update-trainee-goal` `applyGoalWrite`: reads the prior goal under `FOR UPDATE` before overwriting (the only writer that sees old + new goal), and iff the goal genuinely changed (statement or order-insensitive focusAreas) **and** projections exist, re-derives each `stretch := max(stored, deriveStretch(floor, currentTrend))` (upward-only, floor untouched) and stamps `goalLastRenegotiatedAt` (server time) — all atomic with the goal write. Progress deferred to next session-apply (mirrors #296). Pure helpers in `renegotiation.ts` (`isRenegotiation` + `rederiveStretchOnRenegotiation`).

Key calls: trigger detected by server-side content diff (onboarding's first goal-set and the calibration save — goal byte-identical — excluded automatically); upward-only clamp never clobbers a user-raised target or drops on a declining trend; floor stays immovable (a step-up would need its own sign-off → #305); **no SQL migration, no iOS change** (no screen shows projection numbers right after a goal-review save; re-derived stretch reaches the model on the next session sync). The goal write moved from a standalone upsert into a transaction; all prior `upsertGoal` tests still pass. Pinned in **ADR-0022**. Tests: 15 pure unit (green locally) + 6 DB integration (CI `ef-test`); full non-DB goal-EF suite 42 green. Spinoff: **#305** (goal-aware re-derivation / floor step-up, `needs-triage`).

### 2J — Capability-driven projection re-calibration (#305) (2026-06-10)

The payoff #304 deferred — and a re-scope. #305 began as "goal-aware stretch re-derivation (should renegotiation move targets?)". A 3-round grill (each question verified by **two independent reviewer agents** per the #166/#269 process) reshaped it: (a) goal-aware margin (option B) stays deferred — the focus-bump size is a cohort constant with no honest alpha source; (b) the honest, buildable-now lever is the athlete's **own demonstrated capability**, and it's **not** a goal event. So #305 became **"re-calibrate when capability outgrows the band"** (Option D), which **refines ADR-0021/0005's "immovable floor" → "monotonic non-decreasing"**. Pinned in **ADR-0023**.

The 2-agent verification earned its keep: it caught that a *silent* floor bump would reset a just-succeeded athlete's progress + cascade into stretch (→ chose the surfaced re-arm), that the single-boolean ack can't express a repeating event and the model EF's full-object write would revert it (→ chose an event-keyed **watermark**), and that the "trophy-yank" is a screen bug not copy (→ "Levelled up" badge). It also raised a "declining-trend flip-flop" bug that I **verified was a false alarm** (the round-down floor guarantees `stretch > current`), shipping the clean version + an idempotency test instead of dead defensive code.

- [x] S1: #307 — EF mechanism: `recalibration.ts` (`outgrewBand` = capability ≥ stretch + a full band-width, self-relative; `rederiveOutgrownProjection` = floor steps up monotonically to round-down(current), stretch upward-only, progress recomputed; **structurally idempotent**). Wired into the `update-trainee-model` projection arm (per-pattern, after lazy-derive). Stamps the watermark (`lastRecalibratedAtSessionCount` + `lastRecalibratedPatterns`). Exported `roundTo`. 10 unit + 1 orchestrator DB test.
- [x] S2: #308 — surface: `ProjectionState` decodes the watermark; `TraineeModel` gains `acknowledgedRecalibrationSessionCount` (camelCase). `deriveCalibrationReviewSignal` re-arms on watermark > ack; banner/screen get celebratory copy + a "Levelled up" badge naming only the outgrown patterns. Ack advances the watermark (goal EF + client mirror, monotonic). 7 copy + 3 digest + 2 EF DB tests; build-for-testing green.
...
**Design phase — DONE.** Foundation + all five per-screen specs locked and merged: `DESIGN.md` (tokens + decision log) and `docs/design/ui-overhaul-spec.md` (behavior law); `onboarding-calibration.md` (#314), `splash-today.md` (#320), `live-loop.md` (#332), `post-workout.md` (#333), `progress.md` (#336), `train.md` (#338). `ui-overhaul-spec.md` §10 is fully ticked. Key cross-cutting locks: data-honesty law (never fabricate; confidence is visible; every coach line grounded + has a deterministic fallback); the capability band is **one component, three contexts**; **work-is-ink/time-is-pencil** (+plan-is-pencil); the **no-slope law** (sample-and-hold charts); motion restricted to 4 bookends; Train hosts the calibration-review surface (retires the ownerless #269/#305 build debt). Outstanding **design assets**: the caps-APEX wordmark + aperture-A app icon (not yet produced — issue #344).

**Implementation phase — sliced + kicked off (this dispatch).** The §10 final item: broken into **~22 tracer-bullet vertical slices** on the tracker. Strategy = **incremental rollout behind a new 3-tab shell** (old screens stay live until each is swapped); granularity = thin (~2–3 slices/screen); the data/model layer is stable (new screens bind to existing `@Observable` view-models — no backend work). Issue map:
- **Foundation/shared:** #341 design tokens · #342 (HITL) snapshot harness · #343 3-tab shell · #344 (HITL) brand assets · #345 capability-band component · #346 Lens gauge.
- **Daily spine:** #348/#349 Today (hero+Start; state machine) · #350/#351 live loop (core; corrections+edge cases) · #352/#353 post-workout (read+band strip; milestones+amended record).
- **Progress:** #354 root ledger · #355 pattern detail+staircase · #356 session record+period+scrub.
- **Train:** #357 program root · #358 day preview+plan agency · #359 library · #360 detail leaf · #361 (HITL) calibration-review surface.
- **Onboarding + close-out:** #362 onboarding+model reveal · #363 (tracked from day one) strangler close-out.

**Gating architecture decisions** — made via multi-agent panels (3 diverse advisors + 1 independent reviewer per decision + a capstone coherence review; 13 agents), recorded as **ADR-0024 / 0025 / 0026**:
- **0024 (#341):** pure-Swift `DesignSystem` module (no role colorsets; `Theme` via Environment; accent split into a fill-only `ShapeStyle` + a text-ink-no-bright-accent role, P0-4 at compile time; **accent-ink reusable as stroke/dot**; the **data-viz token family + tick/dash geometric constants + dim remap ship in this foundation** — capstone blocker); runtime font registration (not the pbxproj build-setting key); foundation-only (migrate 0 of 236 color sites); one test file.
- **0025 (#342):** adopt swift-snapshot-testing (pinned 1.17+, first SPM dep, test target only); **two layers** (image + cheap geometry/token `@Test` assertions for per-Dynamic-Type honesty invariants); snapshot bare end-state animations-disabled (frame-1==end-state); `APEX_SNAPSHOT_TESTS` gate; **must land after the #341 token+font foundation**; #342 **owns** the `APEX_INTEGRATION_TESTS` testplan-vs-ci.yml contradiction cleanup.
...
A hardening pass on the **current** app (parallel track to the Phase 6 rebuild), resolving the ~40 findings from the 2026-06-10 three-agent UX flow-audit (journey / gym-floor / onboarding; reports in `docs/design/reviews/2026-06-10-flow-audit-*`). Orchestration pattern: plan-architect → critic (decisions pinned in the #318 comment thread) → **8 sequential implementer units**, one branch + one PR each, **independently verified by the orchestrator** (diff read + `xcodebuild build-for-testing` exit 0 + tests) and squash-merged before the next. Umbrella #318 closed via PR #366's `Closes #318`.

- [x] **U1** #334 — deleted stale root `/Features/Workout` duplicates (J-F12, G-F14).
- [x] **U2** #335 — session-start integrity (J-F1, J-F2, O-F3, J-F13c).
- [x] **U3** #337 — honest copy + wired dead-end CTAs across onboarding/program/session flows (J-F4, O-F10, O-F2, J-F5, J-F8/9/11, J-F13a). Extracted the welcome-back message to a tested pure helper.
- [x] **U4** #339 — onboarding answers + **hybrid goal** (digest goal ?? UserDefaults ?? default) reach generation; fixed the UserDefaults-key mismatch that hardcoded every session to "intermediate/hypertrophy"; regenerate-session + manual-entry (`skipToManualEntry`) fixes (O-F4, J-F3/O-F7 cheap parts, O-F6, J-F10). Durable step-persistence deferred → #325.
- [x] **U5** #340 — honest set logging: optional RPE (no preselection), 0-rep floor, skip-set; **load-bearing fix** — last-set determination switched from SetLog-count to set-number-based in both `completeSet` and `skipCurrentSet` (a skip wrote no log → count lagged → phantom extra set) (G-F2, G-F7).
- [x] **U6** #347 — live-loop quick wins: real streak id (`deps.resolvedUserId`), rest-alert-denied notice, rest button polish + m:ss countdown, **real post-workout PRs** (pure `computePersonalRecords`, Epley e1RM over top sets 3–10 reps, history fetched concurrently with the WAQ flush; **suppresses PRs with zero historical baseline** — no fabricated first-session record) (G-F4, G-F5, G-F10, G-F12).
- [x] **U7** #365 — prescription guardrails (largest unit): **one shared `snap → clamp → marker` post-processor** routed from both LLM paths (`handleInferenceResult` + `retryInference`). Snap to available weights per PRD §7.1.1 (`DefaultWeightIncrements.snap`, GymFactStore exclusions, epsilon membership); **runaway clamp** ≤ `snapDown(anchor × 1.15)` for top/backoff only, anchor = last-session top set from a new `cachedLastPerformance` cache refreshed at all 5 transition sites (start/next/skip/swap/resume), upward-only + skipped when no anchor; "Last time" line (`lastPerformanceSummary`); 0-kg fallback seeded from history (no "BW" on a loaded movement) + the preflight→active manual path (critic 7.6 **included** — G-F1 fully addressed, nothing deferred to #321). Marker round-trip pins the `weightAdjustmentNote` string contract. A reentrancy guard re-checks `inferenceGeneration` after the GymFactStore await. (G-F8, G-F3, G-F6, G-F1.) 25 new tests.
- [x] **U8** #366 — banner dismissal semantics: `BannerDismissals` (UserDefaults-backed, injectable suite) replaces 3 transient `@State` flags, keyed by **stable event fingerprints** (welcome-back: last-session date; heavy-reassessment: `triggeringSessionCount`; calibration: the `(calibrationReviewFiredAt, lastRecalibratedAtSessionCount)` watermark pair as Int — never a float). X is local-only (no durable ack). `ContentView` `.task` evaluates the crash sentinel **first**; the migration notice arms + sets its shown-flag only when no crash alert won (SwiftUI was silently dropping one). Full banner-queue redesign deferred → #327. (J-F7 fix-now parts.)

Key calls / data contract: **every unit is client-side + two-step Supabase reads — no SQL migration, no Edge Function change, no prompt change**, so **no pre-deploy steps**. U7 logs raw→adjusted prescription telemetry to console only. Implementer commits trailer Claude Fable 5; U8 was finished + committed by the orchestrator after the implementer hit a session limit mid-task (implementation intact, independently verified). Spinoffs carried: **#327** (banner queue), **#325** (durable onboarding persistence); #321 (offline engine) no longer holds any U7 debt. Not-blocking but worth a manual pass before alpha: the U3/U4 program-complete + regenerate + gym-setup deep-link CTAs (build/test-verified, not in-app QA'd).

## Cross-dimension audit + security/RLS hardening (#369) (2026-06-12)

After #318 closed, a **6-dimension adversarially-verified agent hunt** (security / data-integrity / Edge-Function / concurrency / performance / accessibility) surfaced **29 confirmed findings** beyond the UX-flow reports. Tracked in umbrella **#369**. Implementer agents run **serially** (two parallel `xcodebuild test` runs collide on the simulator and balloon DerivedData — caused a mid-run disk-full incident; clearing `~/Library/Developer/Xcode/DerivedData` recovered it).

**Decision-free bug batch — DONE (each agent-implemented, orchestrator-verified, squash-merged):**
- #371 — P1 dead prescription-accuracy loop: client `TraineeModelSetLogPayload` omitted `ai_prescribed`, so the EF accuracy loop + two deload signals never fired; also fixed `WeekFatigueSignals.setsPerPrimaryMuscle` serializing as a flat array.
- #375 — concurrency: `finishSession` idempotency latch; `inferenceGeneration` guard for retry/swap/resume; atomic `uiSnapshot` replacing a 9-hop torn read in `pullState`.
- #378 — Edge-Function correctness: bound `topSets` via `TOP_SET_RETENTION_COUNT`; guard `fitTransfer` NaN→null on zero-variance; wrap `upsertGoal`'s four writes in one `sql.begin` transaction.
- #374 — performance: single trainee-model decode in `WorkoutView.task`; conditional `LiveSessionWatcher` polling; `ProgressViewModel` cache invalidated on the session counter (orchestrator-corrected — the agent left `invalidateCache()` unwired, which would have hidden a finished workout for 5 min); hoisted `DateFormatter`s; O(N) WAQ flush.
- #377 — security hardening: Keychain `…ThisDeviceOnly`; DEBUG-gate 7 release PII/LLM-response log sites (orchestrator folded in 3 more found by grep; `InferenceSpike` left as dead code).
- #379 — manual sessions set `status="completed"` so they count toward PR baselines (`NULL != 'abandoned'` had silently excluded them).
- #380 — onboarding offers a 2-day-per-week option (the one cheap, non-throwaway onboarding-cluster fix; engine + spec support it).

**Security cluster → Supabase anonymous-auth + enforced RLS — SHIPPED & VERIFIED LIVE (ADR-0027):** the audit proved RLS was OFF on the five core tables (owner policies inert), the client spoke only as `anon` (`authToken` never set), `gym_profiles` had `USING(true)`, and both EFs trusted body `user_id` with no JWT check (IDOR; they connect as `postgres`/BYPASSRLS). Built as 6 slices: #372 anon-auth session · #373 bundle anon key + launch gate (**closed #329** — fresh install can finally onboard) · #382 repoint `resolvedUserId`→`auth.uid()` + onboarding writes `users.id=auth.uid()` · #385 EF JWT `sub==user_id` ownership check (**closes the IDOR**) · #386 the RLS migration `20260612020101_enable_rls_owner_policies.sql` · #388 ADR-0027 + supersede ADR-0016/0018 + CONTEXT. Foundation #368 (bundled Anthropic key + honest "needs setup" gate) + #387 (smoke-test fix). **The Anonymous provider was enabled on the live project** (Management-API `PATCH …/config/auth`; surgical — a blunt `config push` would have clobbered the project's MFA/email-confirmation settings, which `config.toml` had drifted from). **RLS verified live** (Management-API SQL: `relrowsecurity=true` + `FOR ALL` owner policies on all six tables; `gym_profiles` `anon full access` gone). Anon sign-in verified end-to-end (real JWT, `role:authenticated`). Accepted the **alpha data-wipe** (placeholder-keyed rows now invisible; clients must run the #382 build).

**Deploy mechanism (learned):** CI `deploy` job `needs:[ef-test]` (decoupled from the flaky iOS Build&Test) runs `supabase db push` + `functions deploy` on every push to `main` — so a merge IS the deploy. Slice 4 (#385) broke the EF smoke test (POSTed with no auth header → the new gate 401'd) → ef-test red → deploy skipped → the IDOR fix didn't deploy; #387 fixed the smoke harness to send a matching-`sub` Bearer token, re-greening ef-test and re-enabling the auto-deploy.

**Deferred (tracked):** the 6 accessibility / Dynamic-Type findings → the Phase-6 UI rebuild (which replaces those screens). The rest of the onboarding cluster (O-F9 notif timing, O-F12 ordering, full durable mid-flow state) → the #362 onboarding rebuild; O-F11's "remove height/age" is STALE (those fields are consumed by the inference prompt — must NOT be removed). **Follow-ups:** rate-limit/CAPTCHA on anon signups before public launch; reconcile `config.toml` with the live auth settings; harden the intermittent `test_networkFailure_urlError` timing flake.

### Owner-mismatch/auth tail + Phase-3 UI teardown (2026-06-16)

Closing dispatch after the Phase-3 UI was abandoned. Orchestration: 4 read-only scoping agents → per-issue plan → worktree-isolated TDD implementer → **independent review by a separate agent** → orchestrator verify + `--admin` squash-merge. Each fix is one logical unit, one branch, one PR, `Closes #N`.

- **Phase-3 UI removed (PR #426, ADR-0029):** ~9.4k lines / 31 files deleted; app renders the 4-tab `ContentView` unconditionally; `useNewShell` gone; design specs archived to `docs/archive/`. Redesign-era backend (confidence lifecycle, calibration projections, goal renegotiation, recalibration) KEPT.
- **Issue-tracker sweep:** 17 zombie "UI rebuild — Slice N" issues (#342/#343/#348–363/#376) closed `wontfix` (ADR-0029); draft PR #383 closed; **#391 closed** (the `handle_new_user` trigger had already shipped live via #402). **#344 kept open** (APEX wordmark/app-icon — the live app still needs an icon; re-scope outside the UI program).
- **#403** (PR #427) — `performResetAll` now also clears an ACTIVE in-memory `WorkoutSessionManager` session (`resetToIdle()`), via a tiny testable `performLocalStateReset` extraction.
- **#399** (PR #428) — `WorkoutViewModel.startSession` surfaces a "Couldn't start — check your connection and try again" message on the silent auth-gate abort; signature took a `resolveOwner: () async -> UUID?` closure for testability (one caller, `PreWorkoutView`).
- **#409** (PR #429 keystone + #430 tail) — gated the last ContentView-coupled owned writes on the resolved owner. PR-A: `ProgramViewModel` re-resolves the owner at write time (frozen `userId` → injected `resolveOwner` closure; retry re-resolves rather than replays). PR-B: `GymProfileSync.syncIfOwnerResolved` helper for `gym_profiles` (non-fatal deactivate) + inline `resolvedOwnerUserId()` guards on `ProgramDayDetailView`'s set-correction embeds. The three Slice-6 feature saves were already gated (#412).
- **#425** (PR #431) — `ProgramViewModel.loadProgram` now backfills a local-only program to the server once the owner resolves (best-effort, non-blocking; carefully distinguishes "server empty" → backfill from "fetch failed" → bail; reuses `OnboardingProgramPersist.persistIfOwnerResolved`). Covers the onboarding cached-re-entry guard too — no duplicate logic.

**#369 umbrella stays OPEN** for the remaining deferred work: the pre-launch hardening trio (rate-limit/CAPTCHA, `config.toml` reconcile, `test_networkFailure_urlError` flake — none launch-blockers, none started). **The 6 a11y/Dynamic-Type findings are now ORPHANED** — their intended home (the Phase-6 rebuild) was abandoned, so they need a new landing surface against the live 4-tab UI; not yet re-filed.

### Pause-mid-workout flow tightening (2026-06-18)

A user report ("a lot of screens competing and fighting when you pause") drove a panel-designed, agent-built tightening of the live 4-tab pause/resume flow. Diagnosis found three problems: (1) pausing **in place** dropped the user on PreWorkoutView ("Start Workout") because `pausedForThisDay` was only resolved in `WorkoutView.task`, not the `.onChange(.idle)` arm — so the deliberate `WorkoutPausedView` only appeared after a tab round-trip; (2) **two byte-identical pause code-paths** (`onPauseSession` + `onPauseFromRetrySheet`); (3) the one paused sentinel was announced across **6 surfaces in 5 vocabularies**, several with their own Resume button.

**Design:** 4-lens design panel (surgical / iOS-HIG / single-source-of-truth / user-journey) → 3 judges → synthesis locked "**One Pause, One Paused Screen, One Resume Owner**." Rejected the heavier alternatives (new published flag, render-time `load()`, menu reorder) as scope creep. Two copy calls were orchestrator-decided (launch alert "Unfinished Workout"→"Workout paused"; banner button kept "Resume" over the panel's vaguer "Open") — both one-string reverts.

**Shipped as 4 tracer-bullet slices** (each its own branch / PR / `Closes #N`):
- **#465** (PR #469) — extracted a pure `WorkoutView.resolvePausedSentinel(saved:actorIsIdle:trainingDayId:)` shared by the `.task` and `.onChange(.idle)` paths; in-place pause now lands on `WorkoutPausedView`. `load()` called at most once per `.idle` edge (it has a `repairPending` side-effect). Verified safe by the existing `pauseSession()` order: sentinel `.save()` precedes `resetToIdle()`.
- **#466** (PR #470) — `onPauseFromRetrySheet` is now a thin shim to `onPauseSession` (dismisses the sheet first); one pause path. Menu + retry-sheet copy → "Pause workout".
- **#467** (PR #471) — the paused banner's Resume retargets straight to the Workout tab (`applyBannerResume` seam) instead of detouring through a pushed `ProgramDayDetailView`; removed the now-orphaned `navigateToPausedDayDetail` @State + its `.navigationDestination` (grep-confirmed zero refs).
- **#468** (PR #472) — unified all paused copy to **"Workout paused" / "Resume workout" / "Discard workout"** across pill, banner, day-detail, crash-recovery alert, and calendar VoiceOver. The two genuinely-different-state alerts (Session Mismatch, Session Not Found) deliberately excluded.

**Orchestration & the incident:** design + build/review/merge ran as background **Workflow** runs. The implementation workflow's stall-detector (3-min no-output) killed it on slice 4 after 1–3 had already merged — its long `xcodebuild` calls read as "no progress." Recovery: verified 1–3's merged diffs by hand, finished slice 4 directly, and re-ran the suite. **Gotcha:** the test destination pinned `OS=26.2` which this Mac no longer has (only 26.3.1/26.4.1/26.5) — the first run failed `xcodebuild` exit 70 while a naive wrapper reported exit 0; re-ran against `iPhone 17 Pro / OS 26.5` → **TEST SUCCEEDED** (validates all four slices together). All four issues CLOSED.
