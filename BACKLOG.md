...
- [ ] P5-D10: #269 — numeric-projection EDITING on the goal-review screen, deferred from #258 Q1. **UNBLOCKED 2026-06-08** by #166 — pattern confidence now reaches `.established`, so `isReadyForCalibrationReview` can fire. Remaining is #269's own scope: deriving `patternProjections` + setting `calibrationReviewFiredAt` when the review fires, plus the review/edit UX (the confidence feature stopped at the advance-only boundary).
